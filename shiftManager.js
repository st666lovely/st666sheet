...
// Tra ve { off:false, singleTimeOnly:true } neu o chi co 1 moc gio (thieu range) - loi hay gap khi Excel tu doi dinh dang
function parseTimeRangeCell(raw) {
  if (typeof raw === 'number') {
    // Excel luu 1 moc gio don le (vd nhap "12:00" ma khong co dau "-") thanh so thap phan trong ngay
    return { off: false, singleTimeOnly: true };
  }
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    dayCols.forEach(({ index, day }) => {
      ...
      if (parsed.singleTimeOnly) {
        errors.push(
          `Dòng ${r + 1} (${name || telegramId}), ngày ${day}: ô chỉ có 1 mốc giờ (thiếu giờ kết thúc, hoặc Excel đã tự đổi ô thành định dạng Giờ) — cần ghi dạng "HH:mm-HH:mm", đã coi là nghỉ.`
        );
        rowHasError = true;
        daily[day] = null;
        return;
